...
- **Weather** — rolls through on a real-world schedule (rain, fog, overcast), and everyone online sees the same sky.
- **Ride the bus** — a LexTown Loop bus circles the downtown core on a fixed schedule (Main St eastbound → MLK → Vine St westbound → Broadway), pulling in at four stops — Victorian Square, Phoenix Park, the Library, and the Transit Center. Wait at a shelter and the HUD counts down its arrival; when the doors open, press `E` to board and ride to the next stop, `E` again to hop out at the curb. Its position is a pure function of the wall clock — the same trick as the weather — so **everyone sees the same bus in the same place** (private rooms included), with zero new network traffic.
- **Scooter share** — green kick-scooters sit racked at the Transit Center, the courthouse plaza, Triangle Park, and the UK campus edge. Walk up and press `E` to hop on — the missing rung between walking and driving: **about double walk speed** (~34 km/h), nimble at a crawl, rides on grass, and `E` drops it wherever you stop. `W`/`S` throttle and brake, `A`/`D` steer; you stand on the deck the whole time (no jumping or firing while you're on the bars). The scooters themselves are local to each client (like ambient traffic), but **riders are synced** so other players see you glide by on one.
...
- **Mission 8: LOOSE IN THE PADDOCK** — finally, a reason to cross New Circle: the night before the Keeneland yearling sale, three foals jumped the rail at Elmendorf, and the farm foreman needs them settled before the dawn inspectors arrive. Press `E` at the amber ring in the paddock, draw the nerf blaster, and **settle each foal with a calming dart** — one clean hit and it trots itself to the pen (aim ahead of a moving one; darts take a beat to land). Forgiving by design: the start ring is right there in the grass, so a miss is an instant retry. Timed, global board.
- **Mission 9: AIR MAIL** — finally, a reason to master the jetpack: the street's a parking lot and the mail truck rolls at six, so the downtown postmaster straps you into the jetpack to run the day's airmail across the rooftops. Press `E` at the violet ring by spawn, then fly a course that **alternates air-rings (fly through) with rooftop pads (land to bank the stop and refuel)** — because fuel only fills on the ground, the pads *are* the puzzle: you can't reach Big Blue's roof on one tank, so you top off on the Central Bank Tower on the way up. A dry tank or a fall is a time cost, not a reset — only the three-minute clock fails you. Ends standing on top of Big Blue with the whole city below. Timed, global board.
- **Mission 10: HIGH WATER** — a flood watch is up and five of Lexington's notorious low spots are about to go under. The public works yard straps the sandbags on your back: press `E` at the orange ring, then grab **any car** and run them to five **hazard-orange drop rings** — Wolf Run, Kilrush Drive, the downtown underpass, the Tates Creek crossing, and the Vine St dip — before the water crests. **Drive dry:** hydroplaning through standing water costs you time, so mind the puddles. Timed, global board.
- **Mission 11: MOTORCADE** — ripped from the headlines the week a sitting VP flew in to golf: a very important — and conspicuously unnamed — guest has a tee time at Idle Hour, and you're driving the lead car of the motorcade. Press `E` at the red ring at the downtown hotel (Main & Broadway) and run the cleared route east — six red drive-through checkpoints down Vine and through Chevy Chase to the guest gate — against a 180-second clock. The catch is the **composure meter**: every hard knock rattles your passenger, and if it empties he makes you pull over to collect himself while the clock keeps running. Only the tee time fails you. Timed, global board.
- **Mission 12: THE THRILLER** — the series finale. It's the real Main Street Thriller parade, and the mayor offers you a couple of her **'medicinal' gummy bears**; minutes later the two of you watch a **zombie horde** pour out of the October dark and storm the City Hall plaza. Press `E` at the magenta ring and dig in **on foot** — a wave-survival last stand, co-op with the CPU mayor at your side (she never goes down), your weapon **auto-escalating every phase** from machine gun to rocket launcher to grenade launcher until you're squared off against **THE BIG ONE**. Survive it and the hallucination lifts: the 'zombies' were the parade dancers taking their bow, and nobody was ever in danger. Your first clear rolls a **GTA-style credits scene** (skippable, and untimed — your score already froze at the climax). The capstone of the whole run, with its own global board for the fastest full clear.
- **DAILY DASH** — the reason to come back tomorrow: one rotating checkpoint route a day, with its own global board, that **resets at midnight** (a fixed EST anchor, so it flips at the same instant for everyone). Press `E` at the gold ring on the courthouse plaza off Main, then hit **five checkpoints** drawn from a twelve-landmark pool that spans the whole map — downtown parks, Rupp, the Transit Center, UK, Kroger Field, Chevy Chase, the Distillery District, the horse farms up north. The course is generated deterministically from the day's seed, so **everyone in the world runs the exact same five in the same order that day** — argue about it in chat. **Any locomotion counts** — run, drive, jetpack, or catch the Loop bus; the route decides the optimal mix. Your best time for the day is saved on your device, and the modal counts down to the next route.
...
| `E` at a bus stop (doors open) | Ride the **LexTown Loop** bus — wait at a stop, board when the doors open, ride to the next stop; `R` radio, `C` camera, `E` again hops out at the curb |
| `E` at a scooter rack | Grab a **kick-scooter** — W/S throttle, A/D steer, ~double walk speed (racks at the Transit Center, courthouse plaza, Triangle Park, UK); `E` again drops it anywhere |
...
| `E` at the gold ring (courthouse plaza, off Main) | Start the **DAILY DASH** — five checkpoints, new route daily, any locomotion |
...
- **Mission leaderboards** — `{t:'score', ms, m}` submissions are bounds-checked per board (each board has its own plausible-time window, e.g. m1: 3s–10min, m6: 40s–15min) and rate-limited; the top 50 of each persist in `scores.json` and the top 10 are served back via `{t:'scores'}`. Wins are announced in chat. The **DAILY DASH** is the same wiring on a rotating board (`m:10` → the `d` board): the server empties it at the EST day boundary and stamps the response with the current day index, so a stale day never mixes with today's times. The missions themselves run entirely client-side (the mission chopper is a local NPC, not the shared one).
